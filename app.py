...
.cmd-bar{background:linear-gradient(90deg,rgba(0,20,10,.97),rgba(0,15,30,.97));border-bottom:1px solid rgba(0,255,136,.2);padding:0.7rem 1.5rem;display:flex;align-items:center;justify-content:space-between;margin:-0.8rem -1.5rem 1rem -1.5rem;}
...
def gen_sale(ts=None):
    ...
    return {"timestamp":ts or datetime.now(),"order_id":oid,"product":prod,
...
# ── TICKER ────────────────────────────────────────────────────────────────────
tick_items = "   ·   ".join([f"🟢 {r['order_id']} · {r['product']} · {r['city']} · {fi(r['revenue'])}" for _,r in df.head(6).iterrows()])
...
elif trend<-20: alerts.append(("c", f"🚨 CRITICAL DROP [{t}] — Revenue ▼{abs(trend):.1f}%. Immediate investigation in {top_city}."))
elif trend<-8:  alerts.append(("w", f"📉 DECLINE [{t}] — Down {abs(trend):.1f}%. Consider flash discount push."))
...
if rc2: alerts.append(("w",f"🌧️ WEATHER [{t}] — Rain in {', '.join(rc2)}. Boost online deals to offset 30-60% drop."))
