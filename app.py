"""
Revenue Pulse War Room — fixed version
- Removed time.sleep(30) which was blocking chart rendering
- Replaced with st_autorefresh for proper 30s refresh
- Fixed blank Revenue Analytics and Order Feed sections
"""
...
.wr-title { font-family:'Rajdhani',sans-serif; font-size:2.2rem; font-weight:700; color:#10b981; letter-spacing:3px; text-transform:uppercase; }
...
.live-dot { width:7px; height:7px; background:#10b981; border-radius:50%; animation:blink 1.5s infinite; display:inline-block; }
@keyframes blink { 0%,100%{opacity:1;}50%{opacity:0.3;} }
.kpi-card { background:linear-gradient(145deg,#0d1117,#111827); border:1px solid #1f2937; border-radius:12px; padding:1.3rem 1.5rem; position:relative; overflow:hidden; height:135px; }
...
.timestamp { font-family:'Share Tech Mono',monospace; font-size:0.62rem; color:#4b5563; letter-spacing:1px; }
</style>
...
if "sales" not in st.session_state:
    ...
    st.session_state.order_counter = 1001
    st.session_state.last_sale_ts = 0
...
def generate_sale(ts=None):
    ...
    return {
        ...
        "order_id": oid, "product": product, "category": category,
        "price": price, "quantity": quantity, "city": city,
        "region": region, "weather_condition": weather, "revenue": revenue
    }

# Seed initial 50 orders
...
with k4:
    arrow = "▲" if rev_trend >= 0 else "▼"
    cls   = "green" if rev_trend >= 0 else "red"
...
city_list = [c[0] for c in CITIES[:5]]
wdata     = st.session_state.weather_cache
wcc       = st.columns(5)
...
with ch1:
    dft = df.copy()
    dft["bucket"] = dft["timestamp"].dt.floor("15min")
    ...
    fig_r.add_trace(go.Scatter(
        ...
        fillcolor="rgba(16,185,129,0.07)", name="Revenue",
    ...
    fig_r.update_layout(**base_layout("REVENUE OVER TIME (15-MIN BUCKETS)", 280))
...
with ch2:
    ...
    fig_c  = go.Figure(go.Pie(
        ...
        marker=dict(colors=colors[:len(cr)], line=dict(color="#030712", width=2)),
        textfont=dict(size=10, family="Rajdhani"),
        hovertemplate="<b>%{label}</b><br>₹%{value:,.0f}<br>%{percent}<extra></extra>"
    ))
    lo = base_layout("REVENUE BY CATEGORY", 280)
    lo["showlegend"] = True
    lo["annotations"] = [dict(text=f"<b>{top_category}</b>", x=0.5, y=0.5,
                              font=dict(size=11, color="#10b981", family="Rajdhani"), showarrow=False)]
    fig_c.update_layout(**lo)
...
with ch3:
    ...
    fig_ci = go.Figure(go.Bar(
        ...
        marker=dict(color=cir["revenue"],
                    colorscale=[[0,"#064e3b"],[0.5,"#10b981"],[1,"#34d399"]],
                    line=dict(width=0)),
        hovertemplate="<b>%{y}</b><br>₹%{x:,.0f}<extra></extra>"
    ))
    lo_ci = base_layout("REVENUE BY CITY", 260)
    lo_ci["yaxis"]["showgrid"] = False
    fig_ci.update_layout(**lo_ci)
    st.plotly_chart(fig_ci, use_container_width=True)

with ch4:
    wr  = df.groupby("weather_condition")["revenue"].sum().sort_values(ascending=False).reset_index()
    wc2 = ["#10b981" if WEATHER_IMPACT.get(w,("neutral","",""))[0]=="boost"
           else "#ef4444" if WEATHER_IMPACT.get(w,("neutral","",""))[0]=="hurt"
           else "#f59e0b" for w in wr["weather_condition"]]
    fig_w = go.Figure(go.Bar(
        x=wr["weather_condition"], y=wr["revenue"],
        marker=dict(color=wc2, line=dict(width=0)),
        hovertemplate="<b>%{x}</b><br>₹%{y:,.0f}<extra></extra>"
    ))
    lo_w = base_layout("REVENUE BY WEATHER CONDITION", 260)
    lo_w["xaxis"]["tickangle"] = -30
...
with ch6:
    ...
    pivot = dfh.groupby(["city","hour"])["revenue"].sum().reset_index().pivot(
        index="city", columns="hour", values="revenue").fillna(0)
    ...
    lo_h = base_layout("REVENUE HEATMAP · CITY × HOUR OF DAY", 300)
    lo_h["xaxis"]["showgrid"] = False
    lo_h["yaxis"]["showgrid"] = False
...
with ms3:
    bh = dfh.groupby("hour")["revenue"].sum().idxmax() if not df.empty else 12
...
st.markdown('<div style="font-family:Share Tech Mono,monospace;font-size:0.58rem;color:#1f2937;text-align:center;margin-top:2rem;padding-top:1rem;border-top:1px solid #111827;letter-spacing:2px;">REVENUE PULSE WAR ROOM · AUTO-REFRESH 30s · OPENWEATHERMAP INTEGRATED</div>', unsafe_allow_html=True)