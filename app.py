...
.kcard { background:linear-gradient(135deg,rgba(0,20,12,.92),rgba(0,10,20,.92)); border:1px solid rgba(0,255,136,.13); border-radius:4px; padding:1.1rem 1.3rem; position:relative; overflow:hidden; box-shadow:inset 0 1px 0 rgba(0,255,136,.08),0 8px 28px rgba(0,0,0,.55); height:128px; }
...
if not st.session_state.sales:
    now = datetime.now()
    for i in range(120):
        minutes_ago = (120-i)*3 + random.randint(-1,1)
        minutes_ago = max(1, minutes_ago)
        st.session_state.sales.append(gen_sale(now - timedelta(minutes=minutes_ago)))
...
kcard(k1,"g","Total Revenue",fi(tot_rev),"▲ All time cumulative",f"{tot_ord} orders processed","💰")
...
if rc2: alerts.append(("w",f"🌧️ WEATHER [{t}] — Rain in {', '.join(rc2)}. Boost online-only flash deals to offset 30-60% drop."))
hc2 = df[df["weather_condition"]=="Extreme Heat"]["city"].unique().tolist()
if hc2: alerts.append(("w",f"🔥 HEAT [{t}] — Extreme heat in {', '.join(hc2)}. Push home delivery and cooling products."))
cc2 = df[df["weather_condition"]=="Clear"]["city"].unique().tolist()
if cc2: alerts.append(("", f"☀️ OPPORTUNITY [{t}] — Clear skies in {', '.join(cc2[:3])}. Prime time for electronics push. +28% avg conversion."))
...
# CHARTS ROW 1 — KEY FIX: use actual data range, NOT hardcoded 3-hour window
...
with ch4:
    ...
    lo4=clo("REVENUE BY WEATHER",270); lo4["xaxis"]["tickangle"]=-35; lo4["showlegend"]=False
...
with ch7:
    ...
    fig6.update_traces(textfont=dict(family="Share Tech Mono",size=10),
                       hovertemplate="<b>%{label}</b><br>₹%{value:,.0f}<extra></extra>",
                       marker=dict(line=dict(color="#020408",width=2)))
    lo6=clo("REVENUE TREEMAP · REGION × CATEGORY",270)
    lo6["showlegend"]=False; lo6["margin"]=dict(l=4,r=4,t=28,b=4); lo6["coloraxis_showscale"]=False
...
for col,(tag,val,desc) in zip([m1,m2,m3,m4],[
    ("🌤 BEST WEATHER",best_w,"Highest avg revenue. Schedule major campaigns on this condition."),
    ("⏰ PEAK HOUR",f"{peak_hr:02d}:00–{peak_hr+1:02d}:00","Deploy push campaigns 30 min before this window."),
    ("⚠ UNDERPERFORMING",f"{worst_c}",f"Only {fi(worst_r)} revenue. Investigate local factors."),
...
# AUTO REFRESH — no sleep, browser handles it
