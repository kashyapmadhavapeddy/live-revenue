...
.cmd-bar { background:linear-gradient(90deg,rgba(0,20,10,.95),rgba(0,15,30,.95)); border-bottom:1px solid rgba(0,255,136,.2); padding:0.7rem 1.5rem; display:flex; align-items:center; justify-content:space-between; margin:-0.8rem -1.5rem 1rem -1.5rem; }
...
.kcard { background:linear-gradient(135deg,rgba(0,20,12,.92),rgba(0,10,20,.92)); border:1px solid rgba(0,255,136,.13); border-radius:4px; padding:1.1rem 1.3rem; position:relative; overflow:hidden; height:128px; }
...
.arow{display:flex;align-items:flex-start;gap:.5rem;background:rgba(0,255,136,.04);border:1px solid rgba(0,255,136,.13);border-left:3px solid #00ff88;border-radius:3px;padding:.6rem .9rem;margin-bottom:.35rem;font-family:'Share Tech Mono',monospace;font-size:.65rem;color:#88c890;}
...
.ccrd{background:linear-gradient(135deg,rgba(0,15,10,.9),rgba(0,8,18,.9));border:1px solid rgba(0,200,136,.1);border-radius:4px;padding:.85rem .95rem;text-align:center;position:relative;overflow:hidden;}
...
.prow{display:flex;align-items:center;gap:.7rem;background:rgba(0,12,8,.6);border:1px solid rgba(0,255,136,.07);border-radius:3px;padding:.5rem .7rem;margin-bottom:.3rem;}
...
WEATHER = [
    ...
    ("Thunderstorm",0.35,"⛈️"),("Extreme Heat",0.68,"🔥"),
    ("Foggy",0.82,"🌫️"),("Windy",0.88,"💨"),
]
WIMPACT = {
...
# Seed: one sale every 3 minutes for last 6 hours = 120 sales, every bucket filled
...
def chart_base(title="", h=300):
    """Clean base layout — dark bg, green grid, no transparency issues."""
    return dict(
        title=dict(text=title, font=dict(size=11, color="#00cc66", family="Share Tech Mono"), x=0.01),
        plot_bgcolor="#030d06",       # very dark green tint — NOT transparent
        paper_bgcolor="#020408",      # matches page bg
        font=dict(color="#6a9a6a", family="Exo 2", size=10),
        xaxis=dict(
            showgrid=True, gridcolor="#0d2d14", gridwidth=1,
            zeroline=False, zerolinecolor="#0d2d14",
            tickfont=dict(size=9, color="#4a8a5a"),
            showline=True, linecolor="#1a3d20", linewidth=1,
            tickformat="%H:%M",
        ),
        yaxis=dict(
            showgrid=True, gridcolor="#0d2d14", gridwidth=1,
            zeroline=True, zerolinecolor="#1a3d20", zerolinewidth=1,
            tickfont=dict(size=9, color="#4a8a5a"),
            rangemode="tozero",
        ),
        margin=dict(l=10, r=10, t=38, b=10),
        legend=dict(
            bgcolor="rgba(2,4,8,0.8)",
            bordercolor="#1a3d20", borderwidth=1,
            font=dict(size=9, color="#7ab87a"),
...
<div class="cmd-bar">
  ...
  <div style="display:flex;align-items:center;gap:1.2rem;">
    ...
    <div><div class="clk">{now_dt.strftime('%H:%M:%S')}</div>
    <div style="font-family:Share Tech Mono,monospace;font-size:.52rem;color:rgba(0,200,255,.45);letter-spacing:2px;text-align:right;">{now_dt.strftime('%d %b %Y')}</div></div>
  </div>
...
kcard(k5,"p","Today Revenue",fi(today_rev),f"{today_ord} orders",f"Top city: {top_city}","🏆")
...
elif trend < -20: alerts.append(("c", f"🚨 CRITICAL DROP [{t}] — Revenue ▼{abs(trend):.1f}%. Immediate investigation needed in {top_city}."))
elif trend < -8:  alerts.append(("w", f"📉 DECLINE [{t}] — Down {abs(trend):.1f}%. Consider flash discount push to recover."))
else:             alerts.append(("",  f"📈 STABLE [{t}] — Revenue flowing steadily. {top_cat} leading all categories."))
rc2 = df[df["weather_condition"].isin(["Rain","Heavy Rain","Thunderstorm"])]["city"].unique().tolist()
if rc2: alerts.append(("w", f"🌧️ WEATHER [{t}] — Rain in {', '.join(rc2)}. Boost online-only flash deals."))
hc2 = df[df["weather_condition"]=="Extreme Heat"]["city"].unique().tolist()
if hc2: alerts.append(("w", f"🔥 HEAT [{t}] — Extreme heat in {', '.join(hc2)}. Push home delivery offers."))
cc2 = df[df["weather_condition"]=="Clear"]["city"].unique().tolist()
if cc2: alerts.append(("",  f"☀️ OPPORTUNITY [{t}] — Clear skies in {', '.join(cc2[:3])}. +28% avg conversion expected."))
if ord_30 == 0:   alerts.append(("c", f"⛔ PIPELINE DEAD [{t}] — Zero orders in 30 min. Check payment gateway."))
elif ord_30 < 3:  alerts.append(("w", f"⚠️ LOW VOLUME [{t}] — Only {ord_30} orders in 30 min. Deploy push campaign."))
elif ord_30 > 8:  alerts.append(("",  f"⚡ HIGH VELOCITY [{t}] — {ord_30} orders in 30 min. Monitor inventory."))
alerts.append(("",  f"🏆 LEADER [{t}] — {top_cat} dominates. {top_prod} is #1 SKU. {top_reg} outperforming."))
for cls,msg in alerts:
    st.markdown(f'<div class="arow {cls}">{msg}</div>', unsafe_allow_html=True)

# ════════════════════════════════════════════════════════════════════
# CHARTS — all use chart_base() with solid dark background
# ════════════════════════════════════════════════════════════════════
st.markdown('<div class="sec">📊 REVENUE ANALYTICS</div>', unsafe_allow_html=True)
ch1, ch2 = st.columns([2.2, 1])

with ch1:
    # 5-min bucketed revenue timeline
    dft = df.copy()
    ...
    fig.add_trace(go.Scatter(
        x=rt["b"], y=rt["rev"],
        fill="tozeroy",
        fillcolor="rgba(0,180,80,0.12)",
        line=dict(color="#00ff88", width=2.5),
        mode="lines+markers",
        marker=dict(size=5, color="#00ff88", symbol="circle", line=dict(width=1, color="#003320")),
        name="Revenue",
        hovertemplate="<b>%{x|%H:%M}</b><br>Revenue: ₹%{y:,.0f}<extra></extra>",
    ))
    # Moving average
    if len(rt) >= 3:
    ...
    # Orders as bars on secondary y
    fig.add_trace(go.Bar(
        x=rt["b"], y=rt["ord"],
        name="Orders",
        marker_color="rgba(0,180,255,0.25)",
        marker_line_width=0,
        yaxis="y2",
        hovertemplate="Orders: %{y}<extra></extra>",
    ))
    lo = chart_base("REVENUE + ORDER VOLUME  (5-MIN BUCKETS)", 310)
    lo["yaxis2"] = dict(
        overlaying="y", side="right",
        showgrid=False, zeroline=False,
        tickfont=dict(size=8, color="rgba(0,180,255,0.5)"),
        rangemode="tozero",
    )
    lo["bargap"] = 0.05
    lo["xaxis"]["range"] = [rt["b"].min(), rt["b"].max() + timedelta(minutes=5)]
...
with ch2:
    ...
    lo2 = chart_base("CATEGORY REVENUE SPLIT", 310)
    lo2["showlegend"] = True
...
with ch3:
    ...
    fig3 = go.Figure(go.Bar(
        ...
        textposition="inside",
        textfont=dict(size=9, color="#001a08"),
        hovertemplate="<b>%{y}</b><br>₹%{x:,.0f}<extra></extra>",
    ))
    lo3 = chart_base("REVENUE BY CITY", 280)
    lo3["yaxis"]["showgrid"] = False
    lo3["showlegend"] = False
    lo3["xaxis"]["tickformat"] = ",.0f"
...
with ch4:
    ...
    for w in wr["weather_condition"]:
        cls = WIMPACT.get(w,("neutral","",""))[0]
        wcolors.append("#00ff88" if cls=="boost" else "#ff5050" if cls=="hurt" else "#ffaa00")
    ...
    lo4 = chart_base("REVENUE BY WEATHER CONDITION", 280)
    lo4["xaxis"]["tickangle"] = -30
    lo4["xaxis"]["tickformat"] = None
...
with ch5:
    ...
    lo5 = chart_base("REVENUE HEATMAP · CITY × HOUR OF DAY", 280)
    lo5["xaxis"]["showgrid"] = False
    lo5["xaxis"]["tickformat"] = None
...
with ch7:
    ...
    lo6 = chart_base("REVENUE TREEMAP · REGION × CATEGORY", 280)
...
with ch8:
    ...
    lo7 = chart_base("HOURLY REVENUE + ORDERS PATTERN", 280)
    lo7["yaxis2"] = dict(
        overlaying="y", side="right",
        showgrid=False, zeroline=False,
        tickfont=dict(size=8, color="rgba(170,136,255,.5)"),
        rangemode="tozero",
    )
    lo7["xaxis"]["tickformat"] = None
    lo7["xaxis"]["dtick"] = 2
...
for idx,(_,row) in enumerate(df.head(12).iterrows()):
    ...
    rows  += f'<tr><td style="color:rgba(0,255,136,.38);font-size:.56rem;">{row["timestamp"].strftime("%H:%M:%S")}</td><td style="color:rgba(0,200,255,.65);">{row["order_id"]}</td><td>{row["product"]}</td><td style="color:rgba(170,140,255,.65);">{row["category"]}</td><td style="text-align:center;">{row["quantity"]}</td><td>{row["city"]}</td><td class="{wcls}">{row.get("weather_emoji","🌡️")} {row["weather_condition"]}</td><td class="rc">{fi(row["revenue"])}</td></tr>'
st.markdown(f'<div style="background:#030d06;border:1px solid rgba(0,255,136,.1);border-radius:4px;overflow:hidden;"><table class="otbl"><thead><tr><th>TIME</th><th>ORDER ID</th><th>PRODUCT</th><th>CATEGORY</th><th>QTY</th><th>CITY</th><th>WEATHER</th><th>REVENUE</th></tr></thead><tbody>{rows}</tbody></table></div>', unsafe_allow_html=True)
...
for col,(tag,val,desc) in zip([m1,m2,m3,m4],[
    ("🌤 BEST WEATHER", best_w, "Highest avg revenue per order. Schedule major campaigns on this condition."),
    ("⏰ PEAK HOUR", f"{peak_hr:02d}:00–{peak_hr+1:02d}:00", "Deploy push campaigns 30 min before this window for max impact."),
    ("⚠ UNDERPERFORMING", f"{worst_c}", f"Only {fi(worst_r)} revenue. Investigate local weather and campaigns."),
